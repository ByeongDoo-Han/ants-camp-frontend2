...
const nameOf = (code: string) =>
	TICKER_STOCKS.find((s) => s.code === code)?.name ?? null;
...
export function SummaryHero({
	...
	const invested = holdings.reduce(
		(s, h) => s + h.buyPrice * h.stockAmount,
		0,
	);
			...
			<div className="relative z-10 p-6 md:p-7">
				...
				<div className="mb-5">
					...
					<div className="flex items-baseline gap-3 flex-wrap">
						<p
							className="text-3xl md:text-4xl font-bold font-mono tracking-tight leading-none"
							style={{color: t.title}}
						>
							{fmt(asset.totalAssetAmount)}
							<span
								className="text-base ml-1"
								style={{color: t.unitDim}}
							>
								원
							</span>
						</p>
						{invested > 0 && (
							...
							>
								<span className="text-[10px] font-semibold opacity-75 mr-0.5">
									보유 손익
								</span>
								{plUp ? "▲" : "▼"} {plUp ? "+" : ""}
								{fmt(pl)}원 ({plUp ? "+" : ""}
				...
				<div className="flex items-center gap-x-5 gap-y-2 text-xs flex-wrap">
					<div className="flex items-center gap-2">
						...
						<span className="font-mono" style={{color: t.subDim}}>
			...
			<div className="flex items-center gap-3">
				...
				<div className="flex-1 min-w-0">
					...
					<p className="text-[11px] text-gray-500 mt-0.5">
						...
						<span
							className={`font-mono font-semibold ${priceUp ? "text-red-500 dark:text-red-400" : "text-blue-500 dark:text-blue-400"}`}
						>
							{fmt(currentPrice)}
						</span>
						{isPriceFallback && (
